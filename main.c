...
const char *email = "hw@rebelalliance.it";
...
char network_interface_name[8] = "ens33";
...
int receive_dhcp_packet(void *buffer, int buffer_size, int sock, int timeout, struct sockaddr_in *address) {
    ...
    if (!FD_ISSET(sock, &readfds)) {
        ...
        recv_result = recvfrom(sock, (char *) buffer, buffer_size, MSG_PEEK, (struct sockaddr *) &source_address,
                               &address_size);
        if (verbose)
            printf("recv_result_1: %d\n", recv_result);
        recv_result = recvfrom(sock, (char *) buffer, buffer_size, 0, (struct sockaddr *) &source_address,
                               &address_size);
        if (verbose)
...
int get_results(void) {
    ...
    if (request_specific_address == TRUE)
        printf(", requested address (%s) was %soffered", inet_ntoa(requested_address),
...
// -i, --interface=STRING
// 	Interface to to use for listening (i.e. ens33
// -m, --mac=MAC ADDRESS
// 	Set MAC Address
// -v, --verbose
